...
unsafe extern "C" {
    ...
    // pub fn get_current_frame(timestamp: f64) -> *const CFFrame;
    // pub fn free_frame(frame: *const CFFrame);
    pub fn cf_player_open(file_path: *const c_char);
    pub fn cf_player_close();
    pub fn cf_player_get_frame_at(timestamp: i64) -> *const CFFrame;
}
...
impl CFDecoder {
    ...
    pub fn open_video(&self, file_path: &str) {
        unsafe {
            cf_player_open(CString::new(file_path).unwrap().as_ptr());
        }
    }
    ...
    pub fn get_current_frame(&self, timestamp: i64) -> *const CFFrame {
        unsafe {
            cf_player_get_frame_at(timestamp)
        }
    }
