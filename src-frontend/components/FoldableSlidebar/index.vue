    ...
    <div :class="['flex flex-row h-full', direction=='left'? '' : 'flex-row-reverse']">
        ...
        <div class="flex flex-col space-y-2 items-center w-[72px]">
            <div 
                ...
                :class="[
                    'relative group flex-col-center-center w-[58px] h-[58px] text-[11px] text-gray-700 p-2 rounded-[10px] cursor-pointer transition-all duration-300 ease-in-out',
                    activedItem.id == item.id? 'bg-[#FAFAFF]' : 'hover:bg-[#F5F5FA]'
                ]"
            >
                <!-- 高亮块容器 -->
                <div 
                    :class="['absolute top-0 h-full w-[3px] flex items-center justify-center', hlBlockStyle]"
                >
                    <!-- 实际高亮块 -->
...
defineOptions({ name: 'FoldableSlidebar' });
...
const hlBlockStyle = computed(() => {
    return props.direction === 'left' ? 'right-full' : 'left-full';
});
