...
let track: TrackRenderer;
let fileterNode: FilterNode;

let CartoonShaderCode: string;
let CartoonParams: any;
let videoFrame: any; // 使用any类型避免VideoFrame构造函数问题
let isRendering = false; // 添加渲染状态标志

async function onParamsChange(params: any){
    console.log('onParamsChange called with:', params);

    // 防止多个渲染操作同时进行
    if(isRendering) {
        console.warn('Rendering in progress, skipping...');
        return;
    }
    ...
    try {
        ...
        console.log('Params applied, starting render...');
...
onMounted(async () => {
    ...
    // 应用初始参数，确保paramsPack被正确初始化
    fileterNode.applyParams(CartoonParams);
